...
from ffcd import GF256int
from polynomialcd import Polynomial
...
class RSCoder(object):
    def __init__(self, n, k, gen=2):
        ...
        # convert generator to 2
        g = Polynomial((GF256int(1),))
        for alpha in xrange(1,n-k+1):
            p = Polynomial((GF256int(1), GF256int(self.gen)**alpha))
            g = g * p
        self.g = g
        ...
        a = GF256int(self.gen)
        self.a0 = [0 for col in range(256)]
        self.a1 = [ [0 for col in range(self.n)] for row in range(256) ]
        self.a2 = [ [0 for col in range(self.n)] for row in range(256) ]
        self.a3 = [ [0 for col in range(self.n)] for row in range(256) ]
        for j in range(0,self.n):
          for i in range(0,256):
            I = GF256int(i)
            self.a0[i] = I
            self.a1[i][j] = int(I * (a ** j)) 
            self.a2[i][j] = int(I * (a ** (2*j)))
            self.a3[i][j] = int(I * (a ** (3*j)))
        ...
        # change to (32..)
        self.gtimesh = Polynomial(x32=GF256int(1), x0=GF256int(1))

    def process(self, block):
        ok = self.fast_check(block)
        if ok:
          return block, True, False
        else:
          new_block = self.biject(block)
          m = b''
          for i in new_block: m+= chr(i)
          t = self.decode(m,False)
          if len(t) <= self.k:
            new_block = self.ibiject( map(ord, self.encode(t)) )
            return new_block, True, True
          else:
            return block, False, False
    '''
    def fast_check(self, block):
        Z = GF256int(0)
        a = GF256int(self.gen)
        c0, c1, c2, c3 = Z, Z, Z, Z
        j = self.n -1
        for i in block:
          ...
          c3 += I * (a ** (3*j))
          j -= 1
        return (c0==Z) and (c1==Z) and (c2==Z) and (c3==Z)
    '''
    def fast_check(self, block):
        c0, c1, c2, c3 = 0,0,0,0
        j = self.n - 1
    ...
    def biject(self,x):
        ...
        y = []
        for i in x:
          y.append(int( GF256int(x[l-j]) / (a**j)  ))
          j -= 1
        return y
    ...
    def _syndromes(self, r):
        ...
        # α in this implementation is 3 -- converted to 2(self.gen)
        s = [GF256int(0)] # s[0] is 0 (coefficient of z^0)
        for l in xrange(1, n-k+1):
	#SMM
        #for l in xrange(0, n-k):
            s.append( r.evaluate( GF256int(self.gen)**l ) )
